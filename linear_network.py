...
class Junction:
    ...
    def new_riccati(self, road, x):
        """
        Riccati solution using the stored boundary values
        road.rho_l and road.rho_r.

        Assumes solve() has already updated these values.
        """
    ...
    def new_q(self, road, x,b):
        """
        Density using the stored boundary values.
        """

        return self.new_riccati(road, x)
...
def main():
    ...
    R1 = Road(1, rho_l=rho_l_R1, L=1, eta=1.1, lam=1.0, commodities=random_commodities(n_commodities))
    R2 = Road(2, L=1, eta=1.1, lam=1.0, commodities=random_commodities(n_commodities))
    R3 = Road(3, rho_r=rho_r_R3,L=1, eta=1.1, lam=1.0, commodities=random_commodities(n_commodities))
    ...
    commodity_destination_J2 = random_routing(n_commodities, [3])
    ...
    J2 = Junction(
        ...
        incoming_roads=[R2],
        outgoing_roads=[R3],
        commodity_destination=commodity_destination_J2
    )
    ...
    # ========================================================
    # Solve Network (Iterative)
    # ========================================================

    R2.rho_r = 1.0          # Initial guess
    ...
    for it in range(max_iter):
        ...
        # Solve first junction
        J1.solve()

        # Solve second junction
        J2.solve()

        # Check convergence of the shared boundary value
        error = abs(R2.rho_r - old_rho_r)

 

        print(
            f"rho_r(R2) = {R2.rho_r:.12f}"
        )
        ...
        raise RuntimeError("Network iteration did not converge.")
    ...
    def save_all_road_plots(roads, folder_name="Unnamed"):
        ...
        for k, road in enumerate(roads.values()):
            ...
            rho = np.array([
                junction.new_q(road, xi, junction.b)
                for xi in x
            ])
            ...
            axs_density[k].set_ylim(0, 1)
            axs_density[k].set_yticks(np.arange(0, 1.01, 0.1))
            ...
            axs_W[k].set_ylim(0, 1)
            axs_W[k].set_yticks(np.arange(0, 1.01, 0.1))
